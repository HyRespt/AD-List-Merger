...
headers = {
    "User-Agent": "AG-List-Merger/1.0 (compatible; +https://github.com/HyRespt/AG-List-Merger/)"
}
...
header_lines = [
    ...
    "! AG-List-Merger",
    "! Version: 1.0",
    "! Github page: https://github.com/HyRespt/AG-List-Merger/",
    f"! Last modified: {datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
    "!"
]
header = "\n".join(header_lines) + "\n\n"

# Write header + sorted unique domains to file
...
# Final check for duplicates in the resulting file (skipping header lines)
with open(OUTPUT_FILE, "r") as infile:
    lines = [line.strip() for line in infile if line.strip() and not line.strip().startswith('!')]

counter = Counter(lines)
duplicates = [line for line, count in counter.items() if count > 1]

if duplicates:
    total_duplicated_occurrences = sum((count - 1) for _, count in counter.items() if count > 1)
    print(f"\nDuplicates found in the output file! ({total_duplicated_occurrences} total duplicates among {len(duplicates)} distinct lines)")
